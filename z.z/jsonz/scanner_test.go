		...
		t.Run(tcase.Name, func(t *testing.T) {
			...
			for token, err := range Scan(tcase.Data) {
				...
				case "null":
					...
					assert(t, token.typ == TokenNull, "expected null")

				case "true":
					assert(t, err == nil && x == true, "expected true")
					assert(t, token.typ == TokenTrue, "expected true")

				case "false":
					assert(t, err == nil && x == false, "expected false")
					assert(t, token.typ == TokenFalse, "expected false")

				case "1234567890":
					_number0 = true
					assert(t, token.typ == TokenNumber, "expected number")
				...
				case `"\u0123\u4567\u89AB\uCDEF\uabcd\uef4A"`:
					_string0 = true
					assert(t, token.typ == TokenString, "expected string")
